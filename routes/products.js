...
router.get('/select/:offset/:quantityProducts', [
...
router.get('/select/:value', [
	param('value').exists().notEmpty().isString().isLength({ min: 1, max: 200 }).escape()
], (req, res) => {
	const errors = validationResult(req);
	if(!errors.isEmpty()) {
		return res.json('Erros nos parametros');
	...
	sql.query(`SELECT * FROM ss_products WHERE pro_name LIKE ?`, [search], (errors, results, fields) => {
		if(errors) {
			return res.json('Não foi possível realizar a consulta.');
		}
		if(results) {
